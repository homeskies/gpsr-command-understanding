...
from .grammar import CombineExpressions, tree_printer, DiscardVoid
from .util import (get_placeholders, replace_child_in_tree,
    replace_words_in_tree, has_placeholders)
...
def generate_sentences(start_tree, production_rules):
    ...
    while len(stack) != 0:
        sentence = stack.pop()
        replace_tokens = list(sentence.scan_values(lambda x: x in
            production_rules.keys()))
        if replace_tokens:
            ...
            for production in production_rules[replace_token]:
                ...
                replace_child_in_tree(modified_sentence, replace_token,
                    production, only_once=True)
                # Generate the rest of the sentence recursively assuming this
                # replacement
...
def generate_random_pair(start_symbols, production_rules, semantics_rules,
    yield_requires_semantics=False, random_generator=None):
    return next(generate_sentence_parse_pairs(start_symbols, production_rules,
        semantics_rules, yield_requires_semantics=yield_requires_semantics,
        branch_cap=1, random_generator=random_generator))


def generate_sentence_parse_pairs(start_tree, production_rules,
    semantics_rules, start_semantics=None, yield_requires_semantics=True,
    ...
    while not frontier.empty():
        ...
        expansions = list(expand_pair(sentence, semantics, production_rules,
        ...
        if not expansions:
            ...
            if semantics:
                ...
                # Are there placeholders in the semantics that aren't left in
                # the sentence? These will never get expanded, so it's almost
                # certainly an error
                probably_should_be_filled = (sem_placeholders_remaining.
                    difference(sentence_placeholders_remaining))
                if len(probably_should_be_filled) > 0:
                    print("Unfilled placeholders {}".format(" ".join(map(str,
                        probably_should_be_filled))))
                    print(tree_printer.transform(sentence))
                    print(tree_printer.transform(semantics))
                    print("This annotation is probably wrong")
                    print("")
                    continue
                elif len(sem_placeholders_remaining) != len(
                    sentence_placeholders_remaining):
                    not_in_annotation = (sentence_placeholders_remaining.
                        difference(sem_placeholders_remaining))
...
def generate_sentence_slot_pairs(start_tree, production_rules,
    semantics_rules, start_semantics=None, yield_requires_semantics=True,
    branch_cap=None, random_generator=None):
    ...
    while not frontier.empty():
        ...
        expansions = list(expand_pair_slot(sentence, semantics,
            production_rules, semantics_rules, branch_cap=branch_cap,
...
def expand_pair_full(sentence, semantics, production_rules, branch_cap=None,
    random_generator=None):
    return generate_sentence_parse_pairs(sentence, production_rules, {},
        start_semantics=semantics, branch_cap=branch_cap,
        random_generator=random_generator)


def expand_pair(sentence, semantics, production_rules, branch_cap=None,
    random_generator=None):
    replace_token = list(sentence.scan_values(lambda x: x in production_rules.
        keys()))
    ...
    if random_generator:
        ...
        if branch_cap:
            productions = random_generator.sample(replacement_rules,
                k=branch_cap)
        else:
    ...
    for production in productions:
        modified_sentence = copy.deepcopy(sentence)
        replace_child_in_tree(modified_sentence, replace_token, production,
            only_once=True)
        modified_sentence = DiscardVoid().visit(modified_sentence)

        # Normalize any chopped up text fragments to make sure we can pull
        # semantics for these cases
        sentence_filled = CombineExpressions().visit(modified_sentence)
        # If we've got semantics for this expansion already, see if the
        # replacements apply to them For the basic annotation we provided, this
        # should only happen when expanding ground terms

        modified_semantics = None
        if semantics:
            modified_semantics = copy.deepcopy(semantics)
            sem_substitute = production
            if isinstance(replace_token, WildCard) or (len(production.children
                ) > 0 and isinstance(production.children[0], Anonymized)):
                sem_substitute = production.copy()
                sem_substitute.children = ["\""] + sem_substitute.children + [
                    "\""]
            replace_child_in_tree(modified_semantics, replace_token,
                sem_substitute)
        yield sentence_filled, modified_semantics


def expand_pair_slot(sentence, semantics, production_rules, semantics_rules,
    branch_cap=None, random_generator=None):
    #print("sentence: " + sentence.pretty())
    #if semantics:
    #    print("semantics: " + semantics.pretty())
    ...
    if random_generator:
        ...
        replacement_rules = production_rules[replace_token]
        if branch_cap:
            productions = random_generator.sample(replacement_rules, k=min(
                branch_cap, len(replacement_rules)))
        else:
            # Use all of the branches
            productions = production_rules[replace_token]
            random_generator.shuffle(productions)
    ...
    for production in productions:
        modified_sentence = copy.deepcopy(sentence)
        replace_child_in_tree(modified_sentence, replace_token, copy.deepcopy(
            production), only_once=True)
        modified_sentence = DiscardVoid().visit(modified_sentence)
        ...
        if semantics:
            ...
            sem_substitute = get_semantic_substitute(replace_token, copy.
                deepcopy(production), semantics_rules)

            if (len(sem_substitute.children) > 0 and isinstance(sem_substitute.
                children[0], Tree) and sem_substitute.children[0].data ==
                "intent"):
                #copy slot data
                ...
                replace_child_in_tree(modified_semantics, replace_token, slot,
                    only_once=True)
            else:
                replace_child_in_tree(modified_semantics, replace_token,
                    sem_substitute, only_once=True)


            #print("replace_token", replace_token)
            #print("sem_substitute", sem_substitute.pretty())
        else:
            modified_semantics = get_semantic_substitute(replace_token, copy.
                deepcopy(production), semantics_rules)
...
def get_semantic_substitute(replace_token, production, semantics_rules):
    ...
    if sem_production:
        if (len(production.children) == 1) and isinstance(production.children[
            0], WildCard):
            pass
        else:
...
def iob2_tagging(sem, production):
    ...
    if sem:
        ...
        for i, word in enumerate(words):
            tag = Token('WORD', "B-" + sem) if i == 0 else Token('WORD', "I-"
                + sem)
...
def expand_all_semantics(production_rules, semantics_rules):
    ...
    for utterance, parse in semantics_rules.items():
        yield From(generate_sentence_parse_pairs(utterance, production_rules,
...
def pairs_without_placeholders(rules, semantics, only_in_grammar=False):
    ...
    for command, parse in pairs:
        if has_placeholders(command) or has_placeholders(parse):
            ...
            continue
        # If it's important that we only get pairs that are in the grammar,
        # check to make sure
