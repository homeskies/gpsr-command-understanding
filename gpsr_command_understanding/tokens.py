...
class WildCard(NonTerminal):
    ...
    def __str__(self):
        ...
        return "Wildcard(" + '{} {} {}'.format(self.name, type_str, extra_str,
            obfuscated_str).strip() + ')'

    def to_human_readable(self):
        obfuscated_str = '?' if self.obfuscated else ""
        type_str = self.type if self.type else ""
        extra_str = self.extra if self.extra else ""
        return '{' + "{} {} {} {}".format(self.name, type_str, extra_str,
            obfuscated_str).strip() + '}'
    ...
    def __eq__(self, other):
        return (isinstance(other, WildCard) and self.name == other.name and
            self.type == other.type and self.extra == other.extra and self.
