...
from .xml_parsers import (ObjectParser, LocationParser, NameParser,
    GesturesParser)


def load_entities_from_xml(objects_xml_file, locations_xml_file,
    names_xml_file, gestures_xml_file):
    ...
    return (objects, categories, names, locations, beacons, placements, rooms,
        gestures)


def load_wildcard_rules(objects, categories, names, locations, beacons,
    ...
    production_rules[WildCard('location', 'placement', obfuscated= (True)] =
        rooms)
    ...
    things_to_say = ["something about yourself", "the time", "what day it is",
        "the day of the week", "a joke"]
...
def load_all_2018_by_cat(generator, grammar_dir, expand_shorthand=True):
    ...
    cat1_rules = generator.load_rules([common_path, join(grammar_dir,
        "gpsr_category_1_grammar.txt")])
    cat2_rules = generator.load_rules([common_path, join(grammar_dir,
        "gpsr_category_2_grammar.txt")])
    cat3_rules = generator.load_rules([common_path, join(grammar_dir,
        "gpsr_category_3_grammar.txt")])

    cat1_rules_ground = generator.prepare_grounded_rules([common_path, join(
        grammar_dir, "gpsr_category_1_grammar.txt")], entities)
    cat2_rules_ground = generator.prepare_grounded_rules([common_path, join(
        grammar_dir, "gpsr_category_2_grammar.txt")], entities)
    cat3_rules_ground = generator.prepare_grounded_rules([common_path, join(
        grammar_dir, "gpsr_category_3_grammar.txt")], entities)
    cat1_rules_anon = generator.prepare_anonymized_rules([common_path, join(
        grammar_dir, "gpsr_category_1_grammar.txt")])
    cat2_rules_anon = generator.prepare_anonymized_rules([common_path, join(
        grammar_dir, "gpsr_category_2_grammar.txt")])
    cat3_rules_anon = generator.prepare_anonymized_rules([common_path, join(
        grammar_dir, "gpsr_category_3_grammar.txt")])

    cat1_semantics = generator.load_semantics_rules(join(grammar_dir,
        "gpsr_category_1_semantics.txt"))
    cat2_semantics = generator.load_semantics_rules([join(grammar_dir,
        "gpsr_category_1_semantics.txt"), join(grammar_dir,
        "gpsr_category_2_semantics.txt")])
    cat3_semantics = generator.load_semantics_rules(join(grammar_dir,
        "gpsr_category_3_semantics.txt"))

    return [(cat1_rules, cat1_rules_anon, cat1_rules_ground, cat1_semantics),
        (cat2_rules, cat2_rules_anon, cat2_rules_ground, cat2_semantics), (
        cat3_rules, cat3_rules_anon, cat3_rules_ground, cat3_semantics)]
...
def load_all_2018(generator, grammar_dir, expand_shorthand=True):
    ...
    paths = tuple(map(lambda x: join(grammar_dir, x), ["objects.xml",
        "locations.xml", "names.xml", "gestures.xml"]))
    entities = load_entities_from_xml( * paths)
    grammar_files = [common_path, join(grammar_dir,
        "gpsr_category_1_grammar.txt"), join(grammar_dir,
        "gpsr_category_2_grammar.txt"), join(grammar_dir,
        "gpsr_category_3_grammar.txt")]
    ...
    semantics = generator.load_semantics_rules([join(grammar_dir,
        "gpsr_category_1_semantics.txt"), join(grammar_dir,
        "gpsr_category_2_semantics.txt"), join(grammar_dir,
        "gpsr_category_3_semantics.txt")])
...
def load_all_2019(generator, grammar_dir, expand_shorthand=True):
    ...
    rules = generator.load_rules([common_path, join(grammar_dir, "gpsr.txt")],
        expand_shorthand=expand_shorthand)
    rules_ground = generator.prepare_grounded_rules([common_path, join(
        grammar_dir, "gpsr.txt")], entities)
    rules_anon = generator.prepare_anonymized_rules([common_path, join(
        grammar_dir, "gpsr.txt")])
