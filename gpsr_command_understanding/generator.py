...
from .generation import (generate_sentence_parse_pairs,
    generate_sentence_slot_pairs, expand_pair_full)
...
GENERATOR_GRAMMARS = {2018: os.path.abspath(os.path.dirname(__file__) +
    "/../resources/generator_grammar_ebnf.txt"), 2019: os.path.abspath(os.path
    .dirname(__file__) + "/../resources/generator_grammar_ebnf.txt")}

SEMANTIC_FORMS = {"lambda": os.path.abspath(os.path.dirname(__file__) +
    "/../resources/lambda_ebnf.txt"), "slot": os.path.abspath(os.path.dirname(
    __file__) + "/../resources/slot_ebnf.txt")}


class Generator:
    def __init__(self, grammar_format_version=2018,
        semantic_form_version="lambda"):
        with  open(GENERATOR_GRAMMARS[grammar_format_version]) as grammar_spec, open
            (SEMANTIC_FORMS[semantic_form_version]) as annotation_spec:
            grammar_spec = grammar_spec.read()
            annotation_spec = annotation_spec.read()
        self.generator_grammar_parser = Lark(grammar_spec, start='rule_start',
            parser="lalr", transformer=TypeConverter())
        self.generator_sequence_parser = Lark(grammar_spec,
            start='expression_start', parser="lalr", transformer=TypeConverter
            ())
        self.lambda_parser = Lark(annotation_spec, start='start',
            parser="lalr", transformer=TypeConverter())
        self.semantic_form_version = semantic_form_version
        self.rules = []

    def load_set_of_rules(self, grammar_file_paths, semantics_file_paths,
        objects_xml_file, locations_xml_file, names_xml_file,
        gestures_xml_file):
        rules_raw = self.load_rules(grammar_file_paths)
        rules_anon = self.prepare_anonymized_rules(grammar_file_paths)
        rules_ground = self.prepare_grounded_rules(grammar_file_paths,
            objects_xml_file, locations_xml_file, names_xml_file,
            gestures_xml_file)
            ...
            with open(grammar_file_path) as f:
                for line in f:
                    ...
                    lhs, rhs_productions = self.parse_production_rule(line,
                        expand_shorthand)
                    # Skip emtpy LHS (comments) add to dictionary, if already
                    # there then append to list of rules using set to avoid
                    # duplicates
                    if not lhs:
                        continue
    ...
    def parse_rule(self, line, rule_dict):
        ...
        if "=" not in line:
            return
        # TODO: Properly compose these grammars so that we don't have to
        # manually interface them
        ...
        for prod, sem in zip_longest(expanded_prod_heads, expanded_sem_heads,
            fillvalue=expanded_sem_heads[0]):
            # Check for any obvious errors in the annotation
            prod_wildcards = get_wildcards([prod])
            sem_wildcards = (get_wildcards([sem]) if isinstance(sem, Tree) else
                set())

            if sem_wildcards.difference(prod_wildcards):
                raise RuntimeError(
                    "Semantics rely on non-terminal {} that doesn't occur in rule: {}"
                    .format(sem_wildcards, line))
    ...
    def prepare_grounded_rules(self, grammar_file_paths, entities):
        ...
        # This part of the grammar won't lend itself to any useful
        # generalization from rephrasings
    ...
    def prepare_anonymized_rules(self, grammar_file_paths,
        ...
        grounding_rules = make_anonymized_grounding_rules(groundable_terms,
            show_debug_details)
    ...
    def get_utterance_semantics_pairs(self, random_source, rule_sets,
        branch_cap=None):
        all_pairs = {}
        ...
        for rules, rules_anon, rules_ground, semantics in rules:
            ...
            if self.semantic_form_version == "slot":
                pairs = generate_sentence_slot_pairs(ROOT_SYMBOL,
                    rules_ground, semantics, yield_requires_semantics=True,
                    branch_cap=branch_cap, random_generator=random_source)
            else:
                pairs = generate_sentence_parse_pairs(ROOT_SYMBOL,
                    rules_ground, semantics, yield_requires_semantics=True,
                    branch_cap=branch_cap, random_generator=random_source)
...
def get_grounding_per_each_parse(generator, random_source):
    ...
    for rules, rules_anon, rules_ground, semantics in generator:
        # Start with each rule, since this is guaranteed to get at least all
        # possible parses Note, this may include parses that don't fall in the
        # grammar...
        for generation_path, semantic_production in semantics.items():
            # Some non-terminals may expand into different parses (like
            # $oprop)! So we'll expand them every which way
            wild_expansions = list(generate_sentence_parse_pairs(
                generation_path, rules, semantics,
                yield_requires_semantics=True, random_generator=random_source)
                )
            # We're going to be throwing away expansions that have the same
            # parse, so let's randomize here to make sure we aren't favoring
            # the last expansion. Note that the above generation should also
            # return expansions in a random order anyway
            random_source.shuffle(wild_expansions)

            for utterance_wild, parse_wild in list(wild_expansions):
                utterance_anon, parse_anon = next(expand_pair_full(
                    utterance_wild, parse_wild, rules_anon, branch_cap=1,
                    random_generator=random_source))

                utterance, parse_ground = next(expand_pair_full(
                    utterance_wild, parse_wild, rules_ground, branch_cap=1,
                    random_generator=random_source))
                assert not has_placeholders(utterance)
                assert not has_placeholders(parse_ground)
                assert not has_placeholders(parse_ground)
                # We expect this to happen sometimes because of the cat1 cat2
                # object known wildcard situation
                if has_placeholders(parse_anon):
                    continue

                grounded_examples[parse_anon] = (utterance, parse_anon,
                    parse_ground)
