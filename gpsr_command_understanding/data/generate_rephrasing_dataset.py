...
from .generator import Generator, get_grounding_per_each_parse
...
def main():
    ...
    out_file_path = os.path.abspath(os.path.dirname(__file__) +
        "/../../data/rephrasings_data_{}_{}.csv".format(seed,
        groundings_per_parse))
    ...
    with open(out_file_path, 'w') as csvfile:
        ...
        command_columns = [("command" + str(x), "parse" + str(x),
            "parse_ground" + str(x)) for x in range(1, rehprasings_per_hit + 1
            )]
        ...
        for i, chunk in enumerate(chunks):
            if len(chunk) < rehprasings_per_hit:
                ...
                chunk += random_source.sample([pair for chunk in chunks[:i]
                    for pair in chunk], k=needed)
            line = []
            for utterance, parse_anon, parse_ground in chunk:
                line += [tree_printer(utterance), tree_printer(parse_anon),
