...
from .util import (determine_unique_cat_data, save_slot_data, flatten,
    get_pairs_by_cats, merge_dicts)
...
def main():
    ...
    parser.add_argument("-tc", "--test-categories", default=[1, 2, 3],
        nargs='+', type=int)
    parser.add_argument("-p", "--use-parse-split", action='store_true',
        default=False)
    ...
    grammar_file_paths = [common_path, join(grammar_dir,
        "gpsr_category_1_grammar.txt")]
    semantics_file_paths = [join(grammar_dir, "gpsr_category_1_slot.txt"),
        join(grammar_dir, "common_rules_slot.txt")]

    cmd_gen.load_set_of_rules(grammar_file_paths, semantics_file_paths, *
        paths)

    grammar_file_paths = [common_path, join(grammar_dir,
        "gpsr_category_2_grammar.txt")]
    semantics_file_paths = [join(grammar_dir, "gpsr_category_2_slot.txt"),
        join(grammar_dir, "common_rules_slot.txt")]
    ...
    semantics_file_paths = [join(grammar_dir, "gpsr_category_3_slot.txt"),
        join(grammar_dir, "common_rules_slot.txt")]

    cmd_gen.load_set_of_rules(grammar_file_paths, semantics_file_paths, *
        paths)
    # generator = cmd_gen.rules[2] for k,v in generator[2].items(): print(k)
    # print(v) print("---------------------------------------------------------
    # ----------")

    pairs = []
    pairs.append(cmd_gen.get_utterance_semantics_pairs(random_source, [1],
        args.branch_cap))
    pairs.append(cmd_gen.get_utterance_semantics_pairs(random_source, [2],
        args.branch_cap))
    pairs.append(cmd_gen.get_utterance_semantics_pairs(random_source, [3],
        args.branch_cap))
    # pairs = [cmd_gen.get_utterance_semantics_pairs(random_source, [cat],
    # args.branch_cap) for cat in [1, 2, 3]]  if args.turk and
    # len(args.train_categories) == 3: turk_pairs = load_turk_data(args.turk,
    # cmd_gen.lambda_parser) pairs[0] = merge_dicts(pairs[0], turk_pairs)
    ...
    # Randomize for the split, but then sort by utterance length before we save
    ...
    if args.test_categories == args.train_categories:
        # If we're training and testing on the same distributions, these should
        # match exactly
    ...
    info += "Exact split percentage: {:.2f}/{:.2f}/{:.2f} split\n".format(len(
        train) / total, len(val) / total, len(test) / total)
    ...
    with open(meta_out_path, "w") as f:
        ...
        f.write("\n\nUtterance vocab\n")
        for token, count in sorted(utterance_vocab.items(), key=operator.
