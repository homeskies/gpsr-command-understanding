...
from .tokens import WildCard, NonTerminal
...
def has_placeholders(tree):
    return any(tree.scan_values(lambda x: isinstance(x, WildCard) or
        isinstance(x, NonTerminal)))


def get_placeholders(tree):
    return set(tree.scan_values(lambda x: isinstance(x, WildCard) or
        isinstance(x, NonTerminal)))
...
def replace_child_in_tree(tree, child_target, replacement, only_once=False):
    ...
    for tree in tree.iter_subtrees():
        did_replace = replace_child(tree, child_target, replacement,
            only_once=only_once)
        if only_once and did_replace:
            return did_replace
    return did_replace
...
def replace_words_in_tree(tree, replacement):
    for tree in tree.iter_subtrees():
        for i, child in enumerate(tree.children):
            if (isinstance(child, Token) and child.type == 'WORD') or (type(
                child) is str):
                tree.children[i] = replacement
...
def determine_unique_cat_data(cat_data,
    keep_new_utterance_repeat_parse_for_lower_cat=True):
    ...
    for i, cat_pairs in enumerate(cat_data):
        ...
        for utterance, parse in cat_pairs.items():
            ...
            for j, (prev_cat_by_utt, prev_cat_by_parse) in enumerate(zip(
                unique_utterance_pair[:i], unique_parse_pair[:i])):

                # If this utterance was in a prev cat, then we know that
                # neither the utterance nor the parse are unique (because
                # utterances always produce a unique parse)
                ...
                if parse in prev_cat_by_parse.keys():
                    parse_unique_to_cat = False
                    # In that case, we can take the parse to "belong to the
                    # previous category", and tack on this utterance as
                    # training data in category 1
...
def save_slot_data(data, out_path):
    ...
    with open(out_path, "w") as f:
        for sentence, parse in data:
            ...
            width = max(len(x) for x in itertools.chain(sentence_tokens,
                parse_tokens))
            sentence_str = " ".ljust(width + 1) + " ".join(token.ljust(width)
