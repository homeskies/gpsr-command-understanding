...
GRAMMAR_DIR_2018 = os.path.abspath(os.path.dirname(__file__) +
    "/../resources/generator2018")
GRAMMAR_DIR_2019 = os.path.abspath(os.path.dirname(__file__) +
...
class TestGenerator(unittest.TestCase):
    ...
    def test_parse_rule(self):
        rules = {}
        test = self.generator.lambda_parser.parse(
            "(test (lambda $1 :e .(yo 1)))")
        print(test.pretty())
        # parse_rule("$vbgopl to the {room 1}, $vbfind (someone | a person),
        # and answer a {question} = (say (answer {question}) (lambda $1:e
        # (person $1) (at $1 {room 1})))", rules)
    ...
    def test_parse_basic(self):
        test = self.generator.generator_grammar_parser.parse(
            "$test = {pron} went to the mall and {location} $go $home")
        print(test.pretty())

    def test_parse_wildcards(self):
        test = self.generator.generator_sequence_parser.parse("Go to the {location placement 1} and get the {kobject}. Then give it to {name 1} who is next to {name 2} at the {location beacon 1} in the {location room}"
            )
        print(test.pretty())
        self.assertEqual(self.generator.generator_sequence_parser.parse(
            "{location room}"), self.generator.generator_sequence_parser.parse
            ("{room}"))
        self.assertEqual(self.generator.generator_sequence_parser.parse(
            "{location beacon}"), self.generator.generator_sequence_parser.
            parse("{beacon}"))
        self.assertEqual(self.generator.generator_sequence_parser.parse(
            "{kobject}"), self.generator.generator_sequence_parser.parse(
            "{object known}"))

    def test_expand_shorthand(self):
        test = self.generator.generator_grammar_parser.parse(
            "$test    = top choice | (level one (level two alpha | level two beta))"
            )
    ...
    def test_generate(self):
        ...
        semantics = generator.load_semantics_rules(os.path.join(FIXTURE_DIR,
            "semantics.txt"))
        pairs = list(generate_sentence_parse_pairs(NonTerminal("Main"),
            grammar, semantics))
    ...
    def test_load_2019(self):
        ...
        all_2019 = load_all_2019(generator, GRAMMAR_DIR_2019,
            expand_shorthand=False)
